class Var_List:
    ...
    def check_alias(self, ins_addr):
        ...
        if self.rd_analysis.ida_support.print_insn_mnem(ins_addr) == "lea":
            is_alias = 1
        elif self.rd_analysis.ida_support.print_insn_mnem(
            ins_addr
        ) == "mov" and self.rd_analysis.ida_support.is_op_type(
            ins_addr, 1, (register_type,)
        ):
            is_alias = 2
        return is_alias
        ...
        if is_alias == 1:
            ...
            for use_var in use_list:
                if self.rd_analysis.mdis_support.is_mem(use_var[0]):
                    ...
                    search_offset, is_mem = self.rd_analysis.mdis_support.search_int(
                        use_var[0]
                    )
                    register_list = self.rd_analysis.mdis_support.get_mem_register(
                        use_var[0]
                    )
                    break
    ...
    def get_guess_analysis(self):
        ...
        offset = self.rd_analysis.ida_support.get_op_imm_value(
            self.func_analysis.current_ins_addr, 0
        )
        if offset:
            return offset
        offset = self.rd_analysis.ida_support.get_op_imm_value(
            self.func_analysis.current_ins_addr, 1
        )
        return offset
    ...
    def search_case(self, ins_addr, offset_imm):
        ...
        if (
            jmp_addr > self.func_analysis.end_addr
            or jmp_addr < self.func_analysis.start_addr
        ):
            return None, None
        if self.rd_analysis.ida_support.is_bigger(jmp_comm_addr):
            return (jmp_addr, [(offset_imm + 1, 0xEFFFFFFFFFFFFFFF)]), (
                next_addr,
                [(0, offset_imm)],
            )
        elif self.rd_analysis.ida_support.is_not_small(jmp_comm_addr):
            return (jmp_addr, [(offset_imm, 0xEFFFFFFFFFFFFFFF)]), (
                next_addr,
                [(0, offset_imm - 1)],
            )
        elif self.rd_analysis.ida_support.is_not_bigger(jmp_comm_addr):
            return (jmp_addr, [(0, offset_imm)]), (
                next_addr,
                [(offset_imm + 1, 0xEFFFFFFFFFFFFFFF)],
            )

        elif self.rd_analysis.ida_support.is_small(jmp_comm_addr):
            return (jmp_addr, [(0, offset_imm - 1)]), (
                next_addr,
                [(offset_imm, 0xEFFFFFFFFFFFFFFF)],
            )

        elif self.rd_analysis.ida_support.is_eq(jmp_comm_addr):
            return (jmp_addr, [(offset_imm, offset_imm)]), (
                next_addr,
                [(0, offset_imm - 1), (offset_imm + 1, 0xEFFFFFFFFFFFFFFF)],
            )
        elif self.rd_analysis.ida_support.is_uneq(jmp_comm_addr):
            return (
                jmp_addr,
                [(0, offset_imm - 1), (offset_imm + 1, 0xEFFFFFFFFFFFFFFF)],
            ), (next_addr, [(offset_imm, offset_imm)])
        else:
            return None, None
    ...
    def guess_work(self, ins_addr, use_list_all):
        ...
        op_name = self.rd_analysis.ida_support.is_guess(
            self.func_analysis.current_ins_addr
        )  # check cmp == 1 and == 2
        ...
        if op_name == 1:
            ...
            for use_var in use_list_all:
                if self.rd_analysis.ida_support.is_op_type(
                    self.func_analysis.current_ins_addr, 0, (2, 3, 4)
                ) or self.rd_analysis.ida_support.is_op_type(
                    self.func_analysis.current_ins_addr, 1, (2, 3, 4)
                ):
    ...
    def search_from(self, ins_addr, input_var):
        ...
        for var_get_from in var_get_from_list:
            if self.rd_analysis.mdis_support.is_mem(
                var_get_from[0]
            ) and self.rd_analysis.ida_support.is_op_type(input_var[1], 1, (2, 3, 4)):
                return var_get_from
            elif self.rd_analysis.mdis_support.is_register(
                var_get_from[0]
            ) and self.rd_analysis.ida_support.is_op_type(input_var[1], 1, (1,)):
                return var_get_from

    def fix_def(self, ins_addr, def_list, use_list):
        for use_var in use_list:
            if (
                self.rd_analysis.mdis_support.is_mem(use_var[0])
                and self.rd_analysis.ida_support.is_op_type(ins_addr, 1, (2, 3, 4))
                and use_var[1] == -1
            ):
                call_use_list = self.rd_analysis.mdis_support.use_var_search(
                    use_list[0][0]
                )
