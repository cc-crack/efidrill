...
class Indirect_Call(Base_Plugin):
    ...
    def vulnerability_find(self, current_func, use_list, def_list, use_list_all):
        ...
        if self.rd_analysis.ida_support.is_call(current_address):
            if use_list and not self.rd_analysis.ida_support.get_function_address(
                current_func.current_ins_addr
            ):
                if self.check_new_vuln_address(current_address):
                    self.vulnerability_log(
                        current_address,
                        "Noice:indirect function call with outside data address == "
                        + hex(current_address),
                    )
