...
class Smm_Vaild(Base_Plugin):
    ...
    def copy_use_var(self, current_fun, caller_func, use_var):
        ...
        self.smm_vaild_size[current_fun][use_var].extend(
            self.smm_vaild_size[caller_func][use_var]
        )
    ...
    def search_check_size(self, current_func, rdx_var, current_address):
        ...
        if offset:
            ...
            if rdx_var:
                rdx_var_from = self.rd_analysis.function_support.get_var_from(
                    current_func, current_address, rdx_var
                )
                ...
                if (
                    ...
                    has_path_addr = self.rd_analysis.function_support.get_have_path(
                        current_func, current_address
                    )
                    min_list = self.rd_analysis.function_support.get_min_list(
                        current_func, rdx_var_from, has_path_addr
                    )
                    if min_list:
    ...
    def search_data_and_check_size(
        self, current_func, use_list, use_list_all, current_address
    ):

        check_size = None

        rcx_var = self.rd_analysis.function_support.get_var_by_regiter_name(
            current_func, use_list_all, self.rd_analysis.arch.function_param[0]
        )
        rdx_var = self.rd_analysis.function_support.get_var_by_regiter_name(
            current_func, use_list_all, self.rd_analysis.arch.function_param[1]
        )
        if rcx_var:
            rcx_var_from = self.rd_analysis.function_support.get_var_from(
                current_func, current_address, rcx_var
            )
    ...
    def vulnerability_find(self, current_func, use_list, def_list, use_list_all):
        ...
        if self.rd_analysis.ida_support.is_call(current_address) and use_list:
            ...
            self.search_data_and_check_size(
                current_func, use_list, use_list_all, current_address
            )
        ...
        one_var_list = []
        if is_use == 0:
            one_var_list = self.rd_analysis.function_support.get_def_use_data(
                current_func, current_address
            )
        else:
            one_var_list = use_list
        call_register_name = self.rd_analysis.function_support.memory_register_search(
            current_func, mem_var
        )
        for one_var in one_var_list:

            if (
                one_var
                not in self.rd_analysis.function_support.get_interesting_op_list(
                    current_func
                )
            ):
                continue
            ...
            for vaild_memory in self.smm_vaild_size[current_func][one_var]:
                if len(vaild_memory) == 0:
                    if self.check_new_vuln_address(current_address):
                        self.vulnerability_log(
                            current_address,
                            "Vulnerability:SMRAM OOB offset_get == "
                            + hex(offset_get)
                            + " address == "
                            + hex(current_address),
                        )
                ...
                for size, source_path_list in vaild_memory.items():
                    ...
                    for source_path in source_path_list:
                        if self.rd_analysis.function_support.is_have_path(
                            current_func, current_address, source_path
                        ):
                            is_oob = 0
    ...
    def add_interesting_memory_map_list(
        ...
        if "smm_vaild_size" in default_value.keys():
            ...
            self.smm_vaild_size[current_func][def_var].append(
                default_value["smm_vaild_size"]
            )
            return
        ...
        if is_alias and len(use_list) == 1:
            ...
            for use_var in use_list:
                self.smm_vaild_size[current_func][def_var].extend(
                    self.smm_vaild_size[current_func][use_var]
                )
        else:
