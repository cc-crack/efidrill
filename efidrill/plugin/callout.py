...
class Plugin_Callout(Base_Plugin):
    ...
    def vulnerability_find(self, current_func, use_list, def_list, use_list_all):
        ...
        if self.rd_analysis.ida_support.is_call(current_address) and use_list:
            ...
            use_list_call, def_list = self.rd_analysis.function_support.get_use_def(
                current_func, current_address
            )

            call_register_name = (
                self.rd_analysis.function_support.memory_register_search(
                    current_func, use_list_call[0]
                )
            )
            use_list_name = []
            ...
            for use_var in use_list:
                if (
                    self.rd_analysis.mdis_support.is_register(use_var[0])
            ...
            if set(use_list_name) & set(call_register_name):
                ...
                if self.check_new_vuln_address(current_address):
                    self.vulnerability_log(
                        current_address,
                        "Vulnerability:call out find address == "
                        + hex(current_address),
                    )
