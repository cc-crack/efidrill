...
class Function_type:
    ...
    def init_function_analysis(self):
        ...
        self.have_path_dict = self.rd_analysis.mdis_support.mmap_ir_to_address(self.mmap_ir_address,
                                                                               self.have_path_dict)
    ...
    def add_interesting_memory_map_list(self, def_var, use_list=[], ins_addr=-1, is_alias=0, use_list_all=[],
                                        default_value={}):
        mem_use_var = None
        ...
        if def_var not in self.interesting_op_list:
            ...
            self.rd_analysis.pluginmgr.callplugin_on_add_interesting_memory_map_list(self, def_var, use_list, is_alias,
                                                                                     default_value)
            ...
            if sub_function:
                ...
                for use_var in use_list:
                    if self.rd_analysis.mdis_support.is_register(use_var[0]) \
                            and self.rd_analysis.mdis_support.is_useful_register(use_var[0],
                                                                                 self.rd_analysis.arch.function_param):
        ...
        if not self.function_var.guess_work(ins_addr, use_list_all):  # just pass xor
            ...
            if use_list and not self.rd_analysis.ida_support.is_call(self.current_ins_addr):
                ...
                for def_var in def_list:
                    if not self.rd_analysis.mdis_support.is_useful_register(def_var[0],
                                                                            self.rd_analysis.arch.register_list):
    ...
    def check_insteresting_variables(self):
        ...
        while (current_ins_addr < self.end_addr):
            ...
            if self.rd_analysis.ida_support.is_call_name(current_ins_addr, "EFI_SMM_CPU_PROTOCOL.ReadSaveState"):
                ...
                if use_var:
                    def_var = use_var
                    self.add_interesting_memory_map_list(def_var=def_var,
                                                         default_value={'toctou': {-1: []}, "struct": 1,
                ...
                for use_var in self.all_uses[current_ins_addr]:
                    ...
                    if self.rd_analysis.mdis_support.is_register(use_var[0]):
                        if use_var[0].name in self.input_miasm.keys() and use_var[1] == -1:
                            ...
                            self.add_interesting_memory_map_list(def_var=def_var, use_list=father_use_list, is_alias=1,
                    ...
                    if self.rd_analysis.mdis_support.check_is_address(0x40E, use_var[0]):
                        def_var = use_var
                        self.add_interesting_memory_map_list(def_var=def_var,
                                                             default_value={'smm_vaild_size': {0xffffffffffffffff: []},
                                                                            "data_min": 0})
                        # search 40E
                    if self.rd_analysis.ida_support.is_var_name(current_ins_addr, "gBS", 1) or \
                        ...
                        if self.rd_analysis.mdis_support.is_mem(use_var[0]):
                            ...
                            self.add_interesting_memory_map_list(def_var=def_var, default_value={
                                'smm_vaild_size': {0xffffffffffffffff: []}, "data_min": 0})
                            # search gBS
