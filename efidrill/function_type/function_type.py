...
class Function_type:
    ...
    def init_function_analysis(self):
        ...
        self.mmap_ir_address, self.mmap_address_ir =  self.rd_analysis.mdis_support.get_mmap_ir_to_address(self.ircfg)
        self.all_ir = self.rd_analysis.mdis_engine.gather_ir_data(self.ircfg,self.mmap_ir_address)


        self.all_uses, self.all_defs = self.rd_analysis.mdis_engine.get_use_def(self.ircfg, self.mmap_ir_address)
        # Generate rd through mdis in mdis_ engine
        self.def_use = self.rd_analysis.mdis_support.mmap_ir_to_address(self.mmap_ir_address, self.all_uses.def_use)
        self.all_uses = self.rd_analysis.mdis_support.mmap_ir_to_address(self.mmap_ir_address, self.all_uses)
        
        
        self.all_defs = self.rd_analysis.mdis_support.mmap_ir_to_address(self.mmap_ir_address, self.all_defs)

        # gather have path
        self.have_path_dict = self.rd_analysis.mdis_engine.have_path_gather(self.ircfg, self.mmap_ir_address)
        
        self.have_path_dict = self.rd_analysis.mdis_support.mmap_ir_to_address(self.mmap_ir_address, self.have_path_dict)
    ...
    def vulnerability_find(self,  use_list, def_list, use_list_all):           
        self.rd_analysis.pluginmgr.callplugin_on_vulnerability_find(self,use_list, def_list, use_list_all)
    
    def add_interesting_memory_map_list(self, def_var, use_list=[], ins_addr=-1,is_alias=0,use_list_all=[], default_value={}):
        mem_use_var = None
        ...
        if def_var not in self.interesting_op_list:
            ...
            self.rd_analysis.pluginmgr.callplugin_on_add_interesting_memory_map_list(self, def_var, use_list, is_alias, default_value)
    ...
    def get_new_insteresting_use(self, use_list):
        ...
        for use_var in use_list:            
            if  use_var in self.interesting_op_list:
                if self.rd_analysis.mdis_support.is_useful_register(use_var[0], self.rd_analysis.arch.register_list):
    ...
    def fix_call_out(self, ins_addr, use_list, function_deep):
        ...
        if self.rd_analysis.function_callback.function_call_fix(self,use_list,function_deep):
            return
        ...
        if ea:
            sub_function, is_new = self.rd_analysis.check_engine.get_function_by_ea(ea,"SubFunction")
            
            if sub_function:
                is_update_flag = 0
                for use_var in use_list:
                    if self.rd_analysis.mdis_support.is_register(use_var[0]) \
                        and self.rd_analysis.mdis_support.is_useful_register(use_var[0],self.rd_analysis.arch.function_param):
                        ...
                        if use_var not in sub_function.input_miasm[use_var[0].name]: 
                            ...
                            self.rd_analysis.pluginmgr.callplugin_on_copy_use_var(sub_function,self, use_var)
                            # We should copy the status from father function to sub function
                if is_update_flag or (is_new and config.just_data_taint==0):
                    self.rd_analysis.check_engine.function_queue.put((sub_function, function_deep+1))
    ...
    def ana_ins_addr(self, ins_addr, function_deep):
        ...
        if not self.function_var.guess_work(ins_addr, use_list_all):# just pass xor
            ...
            if use_list and not self.rd_analysis.ida_support.is_call(self.current_ins_addr):
                

                self.function_var.fix_def(ins_addr, def_list, use_list)

                for def_var in def_list:
                    if not self.rd_analysis.mdis_support.is_useful_register(def_var[0], self.rd_analysis.arch.register_list):
                        if self.rd_analysis.mdis_support.is_register(def_var[0]):
                            continue
                    self.add_interesting_memory_map_list( \
                        def_var=def_var, use_list=use_list, ins_addr=ins_addr, use_list_all=use_list_all)
            else:
                for def_var in def_list:
                    if not self.rd_analysis.mdis_support.is_useful_register(def_var[0], self.rd_analysis.arch.register_list):
                        if self.rd_analysis.mdis_support.is_register(def_var[0]):
    ...
    def check_insteresting_variables(self):
        ...
        while(current_ins_addr <self.end_addr):
            ...
            if self.rd_analysis.ida_support.is_call_name(current_ins_addr, "EFI_SMM_CPU_PROTOCOL.ReadSaveState"):
                # try:
                    ...
                    if use_var:  
                        def_var = use_var          
                        self.add_interesting_memory_map_list( def_var=def_var, default_value={'toctou':{-1:[]},"struct":1,'smm_vaild_size':{0xffffffffffffffff:[]},"data_min":0})   
            ...
            if current_ins_addr in self.all_uses.keys():
                ...
                for use_var in self.all_uses[current_ins_addr]:
                    ...
                    if self.rd_analysis.mdis_support.is_register(use_var[0]):                  
                        if use_var[0].name in self.input_miasm.keys() and use_var[1] == -1:
                            def_var = use_var
                            father_use_list = self.input_miasm[use_var[0].name]
                            self.add_interesting_memory_map_list( def_var=def_var, use_list=father_use_list, is_alias=1,  default_value={"struct":1})
                    ...
                    if self.rd_analysis.mdis_support.check_is_address(0x40E, use_var[0]):
                        def_var = use_var
                        self.add_interesting_memory_map_list(def_var=def_var, default_value={'smm_vaild_size':{0xffffffffffffffff:[]},"data_min":0})
                        # search 40E
                    if self.rd_analysis.ida_support.is_var_name(current_ins_addr, "gBS",1) or \
                        self.rd_analysis.ida_support.is_var_name(current_ins_addr, "gST",1):
            ...
            current_ins_addr = self.rd_analysis.ida_support.get_next_ins_addr(current_ins_addr )
    ...
    def search_readsavestate(self, search_address):
        ...
        for deep in range(config.deep_size):
            search_address = self.rd_analysis.ida_support.get_prev_ins_addr(search_address)
            ...
            for def_var_one in defs_list:
                ...
                if self.rd_analysis.mdis_support.is_mem(def_var_one[0]):
                    register_list = self.rd_analysis.mdis_support.get_mem_register_name(def_var_one[0])
                    if self.rd_analysis.arch.stack_register[0] in register_list \
                        or   self.rd_analysis.arch.stack_register[1] in register_list or\
                             self.rd_analysis.arch.stack_register[2] in register_list:
                        mem_def = def_var_one
                    break 
