...
class IDA_Support:
    ...
    def is_to_zero(self, address):
        if idc.print_insn_mnem(address) == self.rd_analysis.arch.xor_name:
            return 1
        elif idc.print_insn_mnem(address) == self.rd_analysis.arch.mul_name and idc.get_operand_value(address,1)==0:
            return 1
        elif idc.print_insn_mnem(address) == self.rd_analysis.arch.sub_name and self.is_op_type(address, 0, (1,))\
    ...
    def get_jmp_address(self, ins_addr):
        ...
        return  maybe_address
    def rsp_to_rbp(self, ins_addr, op_index,):
        res = idaapi.insn_t()
