...
class RD_Analysis:
    ...
    def get_dxe_ea(self):
        for ea in self.ida_support.get_function_address_list():
            search_function, is_new = self.check_engine.get_function_by_ea(
                ea, "SubFunction"
            )
            if is_new:
                self.check_engine.function_queue.put(
                    (search_function, config.max_function_deep - 1)
                )
    ...
    def work(self):
        ...
        while 1:
            ...
            logger.info(
                "Begin Function Analyzer, Function address=="
                + hex(ana_function.start_addr)
            )
            logger.info(
                "function_size== "
                + str(len(self.check_engine.ea_function.keys()))
                + " size of ana_function_list =="
                + str(self.check_engine.function_queue.qsize())
            )
    ...
    def init_efiXplorer64(self):
        try:
            ...
            ida_loader.load_and_run_plugin("efiXplorer64", 2)
        except:
            logger.error(
                "Error! Load efiXplorer64 failed! Please check if efiXplorer has been installed correctly."
            )
    ...
    def rd_test(self):
        smi_function, is_new = self.check_engine.get_function_by_ea(
            0xA84, "SubFunction"
        )
